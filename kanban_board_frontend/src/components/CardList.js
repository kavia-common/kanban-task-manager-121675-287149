  ...
  return (
    <div
      ...
      style={{
        minHeight: 34,
        background:
          isOver && canDrop && colCards.length === 0 ? '#22326944' : undefined,
        border:
          isOver && canDrop && colCards.length === 0
            ? '2px dashed #38B2AC'
            : undefined,
        borderRadius:
          isOver && canDrop && colCards.length === 0 ? 7 : undefined,
    ...
    >
      ...
      {adding && (
        <form
          className="kanban-add-card-form"
          onSubmit={async (e) => {
    ...
    drop: async (item) => {
      ...
      if (item.column_id === column.id) {
        ...
        if (movingCard.position !== card.position) {
          ...
          try {
            await Promise.all(
              newOrder.map((c) => updateCard(c.id, { position: c.position })),
            );
          } catch (err) {
            showToast &&
              showToast(
                'Failed to reorder cards: ' + (err.message || err),
                'error',
              );
          }
        }
        ...
        try {
          ...
          showToast &&
            showToast(
              'Failed to move card across columns: ' + (err.message || err),
              'error',
            );
        }
      }
  ...
  return (
    <div
      ...
      style={{
        ...
        transition:
          'background .15s, border .15s, opacity .14s, box-shadow .16s',
      }}
    >

  ...
  const handleConfirmBulkUpload = async (colIdx) => {
    ...
    const cards = entries
      ...
      .filter(
        (card) =>
          card &&
          typeof card.feature === 'string' &&
          card.feature.trim().length > 0,
      );

    if (cards.length === 0) {
      showToast(
        'No valid cards found in the file. Make sure "feature" column is filled.',
        'error',
      );
      return;
    }
    try {
      if (!cards.every((c) => c.feature)) {
        ...
        console.log(
          "[Excel Bulk Upload] One or more mapped cards missing 'feature'",
        );
      }
      ...
      if (error) {
        ...
        showToast(
          `Bulk upload succeeded (${cards.length} cards added)`,
          'success',
        );
      }
    } catch (e) {
      showToast(
        'Bulk upload encountered an error: ' + (e.message || e),
        'error',
      );
    }
  };
          ...
          : ReactDOM.createPortal(
              ...
              >
                ...
                >
                    ...
                    <div style={{ margin: '9px 0' }}>
                      <select
                        style={{
                          ...
                          color: '#292010',
                          border:
                            '1.5px solid var(--color-input-border, #ffb300)',
                        }}
                        onChange={(e) =>
                          handleConfirmBulkUpload(e.target.value)
                        }
                        defaultValue=""
                      >
                  ...
                  >
                    Cards parsed from file:{' '}
                    <strong>{bulkUploadState.entries.length}</strong>
                  </div>
                </div>
              </div>,
              document.body,

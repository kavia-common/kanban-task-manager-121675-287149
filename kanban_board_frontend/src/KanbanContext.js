...
export function KanbanProvider({ children }) {
  ...
  const addCard = async (column_id, cardFields) => {
    ...
    const maxPos = Math.max(
      0,
      ...cards.filter((c) => c.column_id === column_id).map((c) => c.position),
    );
  ...
  const deleteCard = async (id) => {
    ...
    try {
      ...
      if (error) {
        ...
        console.error(
          '[KanbanContext.deleteCard] Supabase delete error:',
          error,
        );
  ...
  const bulkInsertCards = async (column_id, cardsArray) => {
    ...
    // eslint-disable-next-line no-console
    console.log(
      '[KanbanContext.bulkInsertCards] Invoked with column_id',
      column_id,
      ', cardsArray (up to 3):',
      cardsArray.slice(0, 3),
    );
    ...
    if (!Array.isArray(cardsArray) || cardsArray.length === 0) {
      // eslint-disable-next-line no-console
      console.log(
        '[KanbanContext.bulkInsertCards] No cards to insert (empty array).',
      );
    ...
    const existingCardsForColumn = cards.filter(
      (c) => c.column_id === column_id,
    );
    const maxExistingPos =
      existingCardsForColumn.length > 0
    ...
    // eslint-disable-next-line no-console
    console.log(
      '[KanbanContext.bulkInsertCards] Final payload (truncated):',
      payload.slice(0, 5),
      '(total:',
      payload.length,
      ')',
    ...
    // eslint-disable-next-line no-console
    console.log('[KanbanContext.bulkInsertCards] Supabase insert result:', {
      error,
      data,
      status,
    });
    if (error) {
      ...
      // eslint-disable-next-line no-console
      console.error('Supabase bulkInsertCards() error:', error, {
        column_id,
        cardsArray,
        payload,
        data,
      ...
      setError(
        error.message ||
          'Bulk insert error: unable to add cards. Please check your field mapping (due_date format, required fields, etc).',
      );
    } else {
      setError(null);
    }
